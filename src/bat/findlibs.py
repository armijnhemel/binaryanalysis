...
def findlibs(unpackreports, scantempdir, topleveldir, processors, scanenv, batcursors, batcons, scandebug=False, unpacktempdir=None):
    ...
    for i in elffiles:
        ...
        if not i in squashedgraph:
            continue
        filehash = unpackreports[i]['checksum']
        topppname = os.path.join(unpackreports[i]['path'], unpackreports[i]['name'])
        seen = set()
        elfgraph = pydot.Dot(graph_type='digraph')
        if i in plugins:
            rootnode = pydot.Node(topppname, color='blue', style='dashed')
        else:
            rootnode = pydot.Node(topppname)
        ...
        while True:
            ...
            for j in processnodes:
                ...
                if nodetype == "unused":
                    ...
                    #print topppname, ppname, "undeclared"
