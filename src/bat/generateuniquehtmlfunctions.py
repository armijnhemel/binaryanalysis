...
def generateHTML(filename, unpackreport, leafscans, scantempdir, toplevelscandir, envvars={}):
	...
	if leafscans.has_key('ranking'):
		if len(leafscans['ranking']['reports']) != 0:
			htmllinks = []
			for j in leafscans['ranking']['reports']:
				if len(j[2]) != 0:
			...
			if htmllinks != []:
				...
				uniquehtmlfile = gzip.open("%s/%s-unique-function.html.gz" % (reportdir, unpackreport['sha256']), 'wb')
