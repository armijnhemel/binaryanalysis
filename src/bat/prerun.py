...
def verifyWOFF(filename, tempdir=None, tags=[], offsets={}, scanenv={}, debug=False, unpacktempdir=None):
	...
	wofflength = struct.unpack('>L', ttfbytes[4:8])[0]
	...
	ttfbytes = ttffile.read(2)
	if len(ttfbytes) != 2:
		ttffile.close()
		return newtags

	numtables = struct.unpack('>H', ttfbytes)[0]
	...
	if len(ttfbytes) != 2:
		ttffile.close()
		return newtags
	reserved = struct.unpack('>H', ttfbytes)[0]
	if reserved != 0:
		ttffile.close()
		return newtags
	...
	if len(ttfbytes) != 4:
		ttffile.close()
		return newtags
	totalsfntsize = struct.unpack('>I', ttfbytes)[0]
	...
	if len(ttfbytes) != 2:
		ttffile.close()
		return newtags
	majorversion = struct.unpack('>H', ttfbytes)[0]

	ttfbytes = ttffile.read(2)
	if len(ttfbytes) != 2:
		ttffile.close()
		return newtags
	minorversion = struct.unpack('>H', ttfbytes)[0]
	...
	if len(ttfbytes) != 4:
		ttffile.close()
		return newtags
	metadataoffset = struct.unpack('>I', ttfbytes)[0]
	...
	if len(ttfbytes) != 4:
		ttffile.close()
		return newtags
	metadatalength = struct.unpack('>I', ttfbytes)[0]

	## meta data length cannot be larger than the file
	if metadatalength > filesize:
		ttffile.close()
		return newtags

	## meta data length cannot be larger than the file
	if metadatalength + metadataoffset > filesize:
		ttffile.close()
		return newtags
	...
	if len(ttfbytes) != 4:
		ttffile.close()
		return newtags
	metadataoriglength = struct.unpack('>I', ttfbytes)[0]
	...
	if len(ttfbytes) != 4:
		ttffile.close()
		return newtags
	privatedataoffset = struct.unpack('>I', ttfbytes)[0]
	...
	if len(ttfbytes) != 4:
		ttffile.close()
		return newtags
	privatedatalength = struct.unpack('>I', ttfbytes)[0]

	## private data length cannot be larger than the file
	if privatedatalength > filesize:
		ttffile.close()
		return newtags

	## private data length cannot be larger than the file
	if privatedatalength + privatedataoffset > filesize:
		ttffile.close()
		return newtags
	...
	for i in xrange(0, numtables):
		...
		if len(ttfbytes) != 4:
			ttffile.close()
			return newtags
		tabletag = struct.unpack('>I', ttfbytes)[0]
		...
		if len(ttfbytes) != 4:
			ttffile.close()
			return newtags
		tableoffset = struct.unpack('>I', ttfbytes)[0]
		...
		if len(ttfbytes) != 4:
			ttffile.close()
			return newtags
		complength = struct.unpack('>I', ttfbytes)[0]
		if complength > filesize:
			ttffile.close()
			return newtags
		if tableoffset + complength > filesize:
			ttffile.close()
			return newtags
		...
		if len(ttfbytes) != 4:
			ttffile.close()
			return newtags
		uncomplength = struct.unpack('>I', ttfbytes)[0]
