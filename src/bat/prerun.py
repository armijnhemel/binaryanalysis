...
def verifyTTF(filename, tempdir=None, tags=[], offsets={}, scanenv={}, debug=False, unpacktempdir=None):
	...
	if 'compressed' in tags or 'graphics' in tags or 'xml' in tags:
		return newtags
	## a TrueType font file starts with '\x00\x01\x00\x00'
	...
	## Then walk through the font. This is very much like OTF.
	...
	numberoftables = struct.unpack('>H', ttfbytes)[0]
	if numberoftables == 0:
		ttffile.close()
		return newtags
	...
	if len(ttfbytes) != 2:
		ttffile.close()
	searchrange = struct.unpack('>H', ttfbytes)[0]
	...
	entryselector = struct.unpack('>H', ttfbytes)[0]
	...
	rangeshift = struct.unpack('>H', ttfbytes)[0]
	...
	for i in xrange(0,numberoftables):
		...
		tableoffset = struct.unpack('>L', ttfbytes)[0]
		...
		tablelength = struct.unpack('>L', ttfbytes)[0]
		...
		if tablelength + tableoffset > filesize:
			validttf = False
			break
		ttfsize = max(ttfsize, tablelength + tableoffset)
		if ttfsize%4 != 0:
			ttfsize += (4 - ttfsize%4)

		## now calculate the checksum.
		oldoffset = ttffile.tell()
		ttffile.seek(offset+tableoffset)
		ttfbytes = ttffile.read(tablelength)
		if len(ttfbytes) != tablelength:
			validttf = False
			break
		computedchecksum = 0
		pad = 0
		...
		for r in xrange(0, len(ttfbytes)/4):
			computedchecksum += struct.unpack('>L', ttfbytes[r*4:r*4+4])[0]
		...
		if tabletag == 'head':
			ttffile.seek(offset+tableoffset+8)
			ttfbytes = ttffile.read(4)
			if len(ttfbytes) != 4:
				validttf = False
				break

			checksumadjustment = struct.unpack('>L', ttfbytes)[0]
	...
	ttffile.seek(offset)
	computedchecksum = 0
	ttfbytes = ttffile.read(ttfsize)
	for r in xrange(0, ttfsize/4):
