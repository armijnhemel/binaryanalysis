...
The database creation script outputs the database in SQLite format. There is an
ongoing effort to also use PostgreSQL. To convert the database from SQLite to
PostgreSQL there is help script called \texttt{bat-sqlitetopostgresql.py} that
can help convert the database from SQLite to PostgreSQL. A set of statements to
create the database in PostgreSQL can be found in the file
\texttt{maintenance/queries} that can be directly passed to PostgreSQL's
\texttt{psql} program.
