...
check binary files. Its primary application right now is for Open Source software
...
program to help scan Linux kernel images and derive a Linux kernel
...
The main program in BAT is \texttt{bruteforce.py}. It uses a brute
force approach for scanning a binary. It assumes no prior knowledge of how a binary
is constructed or what is inside. Instead it tries to determine what is inside
by applying several methods to find contents in the binary, such as looking for
identifiers and running external tools to find contents in the binary. After
successful identification it then unpacks compressed files, media files
and file systems recursively. Finally a wide range of individual scans
are applied to each extracted file, ranging from determining dynamically linked
libraries to an elaborate ranking method to guess which sources were used to
build a program.

There are two types of scans: unpackers and program/leaf scans.
...
In BAT 5.0 the following file systems, compressed files and media files can be
unpacked:
...
INI format. There is a standard configuration file that enables most of the
...
\texttt{bruteforce.py} outputs its results in XML format on standard output. After
redirecting the output to a file it is possible to look at this file with a
commandline tool such as \texttt{xml\_pp} or a webbrowser such as Mozilla
Firefox.

A graphical user interface is planned that wraps around BAT that will
...
or variants of old ones tend to appear regularly (there are at least 5 or more
...
\item there is no code in the BAT 5.0 that makes use of this information.
\end{itemize}
...
In BAT 5.0 the database is hardcoded to \texttt{/tmp/master}. It can be changed
...
In the latest development versions it is possible to set the location of the database
in the configuration file for \texttt{bruteforce.py}.
...
In BAT 5.0 the ranking scan only searches strings that have their type recorded
as \texttt{C} in the database. This means C and C++ source code and header files,
as well as QML and assembler files.

Other files, such as Java, are ignored. Vice versa, for Java executables (in
the development version of BAT Java scanning has been added) everything but
