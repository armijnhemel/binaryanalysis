...
important things is that there is a database and a user with enough priviliges
to read from and possibly modify the data. After setting up the PostgreSQL
server the following commands could be used to create a database and a user.
It should be noted that for network access settings in PostgreSQL should be
changed. If stricter privileges are needed these commands are probably giving
too many privileges.
