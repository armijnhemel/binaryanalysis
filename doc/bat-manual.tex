...
discussed with a local database adminitrator.

The main configuration file of PostgreSQL is called \texttt{pg\_hba.conf}.
Usually this file resides in the top level PostgreSQL directory, for example
\texttt{/var/lib/pgsql/data} (but this depends on the local configuration). In
this file various configuration options are set, such as how clients can
connect and how they should authenticate.
...
Something similar can be done for the IPv6 connections.

The important things is that there is a database and a user with enough
priviliges to read from and possibly modify the data. After setting up the
PostgreSQL server the following commands could be used to create a database and
a user. It should be noted that for network access settings in PostgreSQL
should be changed. If stricter privileges are needed these commands are
probably giving too many privileges.
