...
\title{Binary Analysis Tool User and Developer Manual - describing version 17}
...
myriad of scans, for example for open source license compliance, visualising
linking information, and so on.  There are also other programs to help with
specific compliance tasks, such as verifying if configurations for a given
BusyBox binary match with the configuration in source code. Also included is a
very experimental program to help scan Linux kernel images and a script to
...
done on Fedora 19 and 20 and Ubuntu 12.04, so those platforms are likely to
...
a commandline tool, while the results can be viewed using a special graphical
viewer.
...
Invoking \texttt{bruteforce.py} is simple. You will need to supply three
parameters:
...
\subsection{\texttt{extractkernelstrings.py} and \texttt{extractkernelconfig.py}}

The two programs \texttt{extractkernelstrings.py} and
\texttt{extractkernelconfig.py} are very
...
be merged into \texttt{createdb.py} in the near future.
...
In BAT 17 the following file systems, compressed files and media files can be
...
Currently (BAT 17) if \texttt{cleanup} is set the files are written directly to
...
C++, assembler, QML, C\#, Java, Scala, JSP, Groovy, PHP, Python and
ActionScript) for string constants, methods, functions, variables and, if
...
is set to \texttt{yes} all tables and indexes will first be dropped.
...
Some Linux distributions (most notably Fedora 18 and later) have standardized on
storing the \texttt{/tmp} file system on tmpfs. This means that part of the system
...
The order of the unpack scans as defined in BAT 17 is:
