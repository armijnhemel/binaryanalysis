...
possible configuration from a Linux kernel image.
...
done on Fedora 17 and 18 and Ubuntu 12.04, so those platforms are likely to
...
for a few things: first, they are used to determine which checks are run. They
...
Tags are used to pass hints to methods that are run later avoid unnecessarily
...
In BAT 13.0 the following file systems, compressed files and media files can be
...
available. In most cases it is completely safe to use multiprocessing, but
there is one important exception: if the ranking method (described later) is
used, without pregenerating the proper caching database (or purchasing one) it
is unsafe to use multiprocessing. If ranking is used and there is a fully
generated caching database it is completely safe and even strongly advised to
use multiprocessing, since it vastly speeds up running the program.
...
\item \texttt{generatereports.py}, \texttt{generateimages.py} and
\texttt{piecharts.py} generate textual and graphical representations of results
of the analysis.
...
\begin{verbatim}
def programScan(path, tags, blacklist=[], envvars=None):
        ## code goes here
\end{verbatim}
...
\begin{verbatim}
def aggregateexample(unpackreports, scantempdir, topleveldir, envvars=None)
\end{verbatim}
...
\begin{verbatim}
def postrunHelloWorld(filename, unpackreport, scantempdir, topleveldir, envvars={}):
        print "Hello World"
\end{verbatim}
...
release (for example \texttt{bat-extratools-13.0}). Make a \texttt{tar.gz}
archive of the directory:
\texttt{tar zcf bat-extratools-13.0.tar.gz bat-extratools-13.0}
\item run \texttt{rpmbuild} to create binary packages:
\texttt{rpmbuild -ta bat-extratools-12.0.tar.gz}
\end{enumerate}
...
\texttt{BAT\_STRINGSCACHE} and \texttt{BAT\_FUNCTIONNAMECACHE} and are
...
The main database currently has 5 tables:
...
scanner. It is not used ottherwise.
...
files, which can save quite some time and declutter the interface.
