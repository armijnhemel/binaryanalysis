...
check binary files by unpacking opaque binary objects
...
Other platforms are only tested on during release time.
...
is the scanner for binary objects, but there are also a few other programs to
help with specific tasks, such as verifying if configurations for the BusyBox
binary match a given binary and a very experimental program to help scan
Linux kernel image and derive a configuration from it.
...
There are two types of scans that are used in BAT: unpack and program/leaf
scans.
...
Two other tools in BAT are \texttt{busybox.py} (in the subdirectory
\texttt{bat}) and \texttt{busybox-compare-configs.py}. These two tools are
...
Afterwards \texttt{busybox-compare-configs.py} is used to compare the extracted
configuration with a vendor supplied configuration.
...
value is set to \texttt{/etc/bat}, but this might change in the future.
...
\subsection{\texttt{extractkernelstrings.py}, \texttt{extractkernelconfig.py} and \texttt{findkernelstrings.py}}
