...
Other platforms (Debian 7, Ubuntu 12.04 and later) are tested with, but only to
...
In this section a short overview of the configuration and inner workings are
given. A more detailed description, including how to write scans, can be found
in the appendix.
...
The first action performed is scanning a file for known markers of compressed
files and file systems. Which markers are searched for depends on the contents
of the configuration file (specifically the \texttt{magic} attribute). These
markers determine other methods, such as unpackers, are run.
...
For example, files that only contain text are tagged as \texttt{text}, other
...
that only work on binaries can ignore anything that has been tagged with
\texttt{text}.

Other checks that are available are for valid XML, various Android formats,
ELF executables and libraries, graphics files, and so on.
...
The post-run methods have the type \texttt{postrun} in the configuration.
...
as pictures and more reporting. These dumps are meant to be used by
\texttt{batgui}.
...
shown. If the ranking method was enabled results of the ranking process will
be displayed as well.

In advanced mode more results will be shown, such as a graphical representation
of a file, where every bit in the binary has been assigned a grayscale value,
plus a textual representation of a file generated with \texttt{hexdump}.
Advanced mode is disabled by default, since loading the additional pictures and
data is quite resource intensive and it will only be useful in very specific
cases.
...
\begin{itemize}
\item \texttt{bruteforce.py} contains the main logic of the program: it
...
corrected.
\item \texttt{fssearch.py} defines convenience functions for searching for
certain headers. It is not widely used and likely to be removed in future
versions.
\item \texttt{fwunpack.py} includes most of the functionality for unpacking
compressed files and file systems.
\item \texttt{generatehexdump.py}, \texttt{generateuniquehtml.py} and
\texttt{images.py} have code to generate textual and graphical representations
of input files and results of the analysis.
...
\item \texttt{prerun.py} contains scans that are run in the pre-run phase
for correctly tagging files as early in the process as possible.
...
Package building and testing is done on Ubuntu 12.04.% and Debian 7.
...
\texttt{generatelist.py}, unpacks the files (gzip compressed TAR, bzip2
compressed TAR, XZ compressed tar and ZIP are currently supported) and scans
each individual source code file (at the moment just C/C++/assembler/QML files,
assembler files, C\# files, Java/Scala/JSP/Groovy files and ActionScript files)
for strings and, if enabled, licenses using Ninka.
...
scanner, with support for the Nomos license scanner from FOSSology being added
in the near future. This option is disabled by default for a few reasons:

\begin{itemize}
\item scanning licenses costs significantly more time
...
envvars     = BAT_SQLITE_AVG_C=/home/armijn/master/avg:
              BAT_SQLITE_STRINGSCACHE_C=/home/armijn/master/stringscache_c:
              BAT_SQLITE_DB=/home/armijn/master/master:
              BAT_SQLITE_FUNCTIONNAME_CACHE=/home/armijn/master/funccache:
...
code from both languages the caching databases were split. This makes the
...
\texttt{BAT\_SQLITE\_STRINGSCACHE} and \texttt{BAT\_SQLITE\_AVG} and are
postfixed with an underscore and the name of the programming language family.
The strings cache database for Java for example is configured using the environment
variable \texttt{BAT\_SQLITE\_STRINGSCACHE\_JAVA}.

The entire database with all information is \texttt{BAT\_SQLITE\_DB}. If there
is no value supplied via the environment it will use a hardcoded value
(although this is likely to be removed in a future version):
