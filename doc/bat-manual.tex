...
Other platforms are tested with, but only to build binary releases.
...
invest in more memory and faster disks than in raw CPU power.
...
how a binary is constructed or what is inside. Instead it tries to determine
what is inside by applying several methods to find contents in the binary, such
as looking for identifiers and running external tools to find contents in the
binary. After successful identification it unpacks compressed files, media
files and file systems recursively. Finally a wide range of individual scans
...
can be searched for in a binary blob. Using this information it
...
after unpacking, including files that contained files that were found
