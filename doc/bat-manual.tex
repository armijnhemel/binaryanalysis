...
Adding 4. is really hard to do in a generic way, if not impossible because often no clear markers are recorded into the binary. A possibility would be to process the BusyBox sources, record which strings are unique to certain features and make checks accordingly. For example (from busybox 1.15.3, \texttt{editors/diff.c}):
...
There are also a few applets in 1.1.0 which are a bit harder to detect: \texttt{busybox}, \texttt{mkfs.ext3}, \texttt{e3fsck} and \texttt{[[}. These could be added by hand, since there are just four of them.
