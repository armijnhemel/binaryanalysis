...
\item process results from scans in step 5
\item pack results
\end{enumerate}
...
In BAT 10.0 the following file systems, compressed files and media files can be
...
\item \texttt{generatehexdump.py} and \texttt{images.py} generate textual
and graphical representations of the input files.
...
currently supported) and scans each individual source code file (at the moment
just C/C++/assembler/QML files, assembler files, C\# files,
Java/Scala/JSP/Groovy files and ActionScript files) for strings and, if
...
The database currently has 6 tables:
