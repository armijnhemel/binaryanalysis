...
The following example is for unpacking of 7z archives. The priority is very low, only the identifier for \texttt{7z} is used, and a bunch of file types can safely be ignored by this scan:
...
\begin{itemize}
\item amount of recognized strings
\item distribution of recognized strings
\end{itemize}

%If there are only few strings that can be recognized, the results are likely to be not very reliable.
}
