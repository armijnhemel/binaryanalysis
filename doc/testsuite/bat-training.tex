...
\begin{itemize}
\item a computer running a recent installation of Fedora (version 14 or higher), Debian 7, or Ubuntu (version 12.04 or later)
...
\item lightweight
\item uses standard Linux tools, or standard tools with some slight tweaks
...
\item extract human readable strings from programs and compare this to (publicly available) source code
\item use other information like file names, other files, etc. for circumstantial evidence
\end{itemize}

If you can relate many strings from a binary file to source code it becomes statistically hard to deny (re)use of software.
}
...
Firmwares are not enough for a proper analysis of a \textit{device}:

\begin{itemize}
\item not all software that is on the device is in the firmware (bootloader, recovery, etc.)
\item firmware might be encrypted
\item no firmware available
\end{itemize}

Via other methods (serial port, security holes) you need to gain access to the device to get
access to the full contents of the flash chip.

Obtaining firmwares via these methods is out of scope for this course.
}
...
To understand what BAT does under the hood it is good to do it yourself manually a few times. We will look at:
...
Displaying files in a readable format is essential for manual analysis.
}
...
Many compressed files and file systems start, or end, with identifiers.

\begin{itemize}
\item \texttt{PK} for ZIP files
\item \texttt{ELF} for ELF files
\item \texttt{0x1f 0x8b 0x08} for gzip compression
\item \dots
\end{itemize}

Many identifiers are well documented, for example in \texttt{/usr/share/magic} or file specification.
}
...
If there are large amounts of NUL characters it often indicates a partition boundary.

For example, here the padding directly precedes a SquashFS file system.
\end{frame}
...
\begin{enumerate}
\item use \texttt{hexdump -C} to display the contents of the file
\item report likely offsets for gzip compressed file and SquashFS file system
\end{enumerate}
}

\frame{
\frametitle{Extracting files}
After finding the right offsets you need to carve out the right parts of the file:
...
\frame{
\frametitle{Verifying and unpacking files}

\begin{itemize}
\item remove all bytes preceding an identifier
\item run an unpacker tool
\item verify contents were successfully unpacked
\end{itemize}
}
...
\frame{
\frametitle{Automated extraction using BAT}
Manually extracting files is a lot of work and it is easy to miss files. BAT automates extraction:
...
First make sure BAT (version 6 or higher), plus all necessary dependencies are installed. Then run:

\texttt{bruteforce.py -c /path/to/configuration -b /path/to/firmware > /path/to/output}
}

\frame{
\frametitle{Reading and interpreting results}

The output from BAT is in XML format. To display it use a command like this:

\texttt{xml\_pp /path/to/bat/output | less}

or use a web browser such as Firefox to navigate the XML file.

In the near future a GUI for easier navigation of the results will be made.
}
