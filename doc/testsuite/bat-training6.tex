...
There is a script for generating a database called \texttt{batchextractprogramstrings.py}. It can be found in the source repository of BAT in the directory \texttt{maintenance}. It is a bit of a misnomer, because it can extract a lot more than string constants:

\begin{itemize}
\item string constants
\item function names
\item license information (using Ninka)
\end{itemize}
...
If license information should be extracted Ninka should first be installed. The \texttt{-l} flag should be added to the above command.
}
...
The parameters for the database are passed via the environment:
